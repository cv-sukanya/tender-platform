...
async function checkTenders() {
    try {
        ...
        for (let tender of tenders) {
            ...
            tender.document_start_date = formatDate(tender.document_start_date);
            tender.document_end_date = formatDate(tender.document_end_date);
            tender.clarification_start_date = formatDate(tender.clarification_start_date);
            tender.clarification_end_date = formatDate(tender.clarification_end_date);

            function isRecentTender(dateStr) {
                if (!dateStr) return false;

                const tenderDate = new Date(dateStr);
                const now = new Date();

                const diffDays = (now - tenderDate) / (1000 * 60 * 60 * 24);

                return diffDays <= 2; // last 2 days only
            }

            if (!isRecentTender(tender.publish_date)) {
                console.log("Skipping old tender:", tender.title);
                continue;
            }
            ...
            if (tender.tender_value) {
                ...
                // Handle NA values
                if (
                    tender.tender_value.toUpperCase() === "NA" ||
                    tender.tender_value === ""
                ) {
            ...
            // Period of work 
            if (tender.period_of_work_days) {

                tender.period_of_work_days = String(tender.period_of_work_days)
                    .replace(/[₹,]/g, "")
                    .trim();

                // Handle NA values
                if (
                    tender.period_of_work_days.toUpperCase() === "NA" ||
            ...
            db.query(checkQuery, [tender.tender_ref_no], (err, result) => {
                ...
                if (result.length === 0) {
                    ...
                    console.log([
                        ...
                        tender.document_start_date,
                        tender.document_end_date,
